...
const WIDTH = 20;
const HEIGHT = 20;
const bladeRamp = createColorRamp([
	rgba(71, 86, 108),
	rgba(156, 178, 201),
	rgba(238, 244, 250),
]);
const hiltPalette = createPalette([
	rgba(60, 33, 18),
	rgba(109, 71, 39),
	rgba(179, 130, 71),
]);

export const swordExampleProject: ExampleProject = {
	id: "sword",
	width: WIDTH,
	height: HEIGHT,
	render(): PixelCanvas {
		...
		for (let y = 4; y < 13; y += 1) {
			for (let x = 8; x < 12; x += 1) {
				...
				const light = Math.min(
					1,
					0.2 + verticalFade * 0.4 + (1 - distanceFromCenter) * 0.5,
				);
