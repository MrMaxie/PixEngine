...
const foliageRamp = createColorRamp([
	rgba(26, 56, 28),
	rgba(57, 105, 55),
	rgba(102, 153, 76),
	rgba(181, 214, 129),
...
export const treeExampleProject: ExampleProject = {
	id: "tree",
	width: WIDTH,
	height: HEIGHT,
	render(): PixelCanvas {
		...
		for (let y = 3; y < 18; y += 1) {
			for (let x = 3; x < 21; x += 1) {
				...
				const noiseValue = sampleFractalNoise2D(
					canopyNoise,
					x / 7 + 0.11,
					y / 6 + 0.47,
					{
				...
				const light = Math.min(
					1,
					0.25 + (1 - Math.max(0, dy)) * 0.45 + noiseValue * 0.3,
				);
