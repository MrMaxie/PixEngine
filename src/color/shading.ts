...
import { type ColorRamp, type Palette, sampleColorRamp, samplePalette } from './palette.ts';

export const shadeRamp = (ramp: ColorRamp, light: number) => sampleColorRamp(ramp, light);

export const shadePalette = (palette: Palette, light: number) => samplePalette(palette, light);

export const lambertLight = (normalX: number, normalY: number, lightX: number, lightY: number, ambient = 0) => {
...
const normalizeVector = (x: number, y: number, name: string) => {
